...
- The
  ...
  sub-quadratic consolidation test takes the best of three timings for
  each of its two workloads rather than a single sample. Elapsed time is
  only ever inflated by noise, so the minimum is the robust estimate; a
  lone sample of a sub-second workload drifted far enough on shared CI
  runners to cross the threshold on its own (ratios of 3.35 and 4.30
  against a settled value of ~2.15). The threshold is unchanged, since 4
  is the quadratic ratio the test exists to catch.
