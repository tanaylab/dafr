...
test_that(".path_for_* builds store paths", {
    root <- "/tmp/store"
    expect_equal(
        dafr:::.path_scalar(root, "pi"),
        "/tmp/store/scalars/pi.json"
    )
    expect_equal(
        dafr:::.path_axis(root, "cell"),
        "/tmp/store/axes/cell.txt"
    )
    expect_equal(
        dafr:::.path_vector_dir(root, "cell"),
        "/tmp/store/vectors/cell"
    )
    expect_equal(
        dafr:::.path_matrix_dir(root, "cell", "gene"),
...
test_that(".read_scalar_json returns typed value", {
    tmp <- tempfile(fileext = ".json")
    writeLines('{"type":"Float64","value":3.14}', tmp)
    on.exit(unlink(tmp))
...
test_that(".should_sparsify_numeric bool threshold", {
    expect_true(dafr:::.should_sparsify_numeric(
        c(rep(FALSE, 90), rep(TRUE, 10)),
        "Bool", "UInt32"
    ))
    expect_false(dafr:::.should_sparsify_numeric(
        c(rep(FALSE, 80), rep(TRUE, 20)),
        "Bool", "UInt32"
    ))
})
