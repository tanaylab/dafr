...
# `[ metacell ]` mask while being correctly dropped by other paths.
#
# These assertions check value sets only, not names, because dev's
# query layer doesn't preserve them yet (a separate gap).
...
test_that("[ string_prop ] drops entries whose value is the empty string", {
    ...
    expect_equal(sort(unname(out)), c(10L, 20L, 30L, 40L))
})
...
test_that("[ ! string_prop ] keeps only entries whose value is the empty string", {
    ...
    expect_equal(sort(unname(out)), c(50L))
})
...
test_that("numeric mask still drops zeros", {
    ...
    expect_equal(length(out), 1L)
    expect_equal(sort(unname(out)), c(20L))
})
