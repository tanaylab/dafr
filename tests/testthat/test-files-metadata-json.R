...
test_that("files_daf writable open + set_*/delete_* maintain metadata.json, no zip", {
    root <- withr::local_tempdir()
    d <- files_daf(root, mode = "w+", name = "m")
    add_axis(d, "cell", paste0("c", 1:3))
    ...
    expect_equal(m[["scalars/title"]], list(type = "String", value = "hi"))
    expect_equal(m[["axes/cell"]], list(format = "axis", n_entries = 3L))
    ...
    # reopen reads back fine (local tree-walk, unaffected)
    ro <- files_daf(root, mode = "r")
    expect_equal(get_scalar(ro, "title"), "hi")
})
