...
# Reduction op builders. Canonicalise as `>> <Name>` (`ReduceToScalar` AST
# node) — Julia parity. The eltwise dispatch can't run these (the registry
# has them only as reductions), so emitting `Eltwise` nodes from the
# builders would be runtime-broken anyway.
.qop_reduction_typed <- function(name, type, params) {
    if (!is.null(type)) {
        params <- c(list(type = type), as.list(params))
    }
    if (length(params)) {
    ...
    .qop_reduce_to_scalar(name, params = params)
}
.qop_max <- function() .qop_reduce_to_scalar("Max")
.qop_min <- function() .qop_reduce_to_scalar("Min")
.qop_mode <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Mode", type, params)
}
.qop_sum <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Sum", type, params)
}
.qop_mean <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Mean", type, params)
}
.qop_median <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Median", type, params)
}
.qop_count <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Count", type, params)
}
.qop_geomean <- function(type = NULL, params = list()) {
    .qop_reduction_typed("GeoMean", type, params)
}
.qop_quantile <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Quantile", type, params)
}
.qop_std <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Std", type, params)
}
.qop_std_n <- function(type = NULL, params = list()) {
    .qop_reduction_typed("StdN", type, params)
}
.qop_var <- function(type = NULL, params = list()) {
    .qop_reduction_typed("Var", type, params)
}
.qop_var_n <- function(type = NULL, params = list()) {
    .qop_reduction_typed("VarN", type, params)
}
