...
#' supports `memory://` (or no path / NULL) for in-memory stores and
#' regular filesystem paths for `files_daf`. Future backends are
#' stubbed with explicit error messages: `*.daf.zarr` /
#' `*.daf.zarr.zip` errors with "lands in slice 16",
...
open_daf <- function(uri = NULL, mode = "r", name = NULL, ...) {
    ...
    if (grepl("\\.daf\\.zarr(\\.zip)?$", uri) ||
        grepl("\\.daf\\.zarr\\.zip#", uri, fixed = FALSE)) {
        stop(sprintf(
            "open_daf: zarr backend (%s) lands in slice 16; not yet supported",
            sQuote(uri)
        ), call. = FALSE)
    }
    if (grepl("^https?://", uri)) {
