---
title: "Zarr and HTTP storage"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Zarr and HTTP storage}
...
* **`zarr_daf("http(s)://...")`** — read a `.daf.zarr` directory served
  ...
  of dafr's internal `ZarrStore` interface. Fetches `.zmetadata` once
...
`options(dafr.http_timeout = N)` or env `DAFR_HTTP_TIMEOUT`. There is
no automatic retry; flaky-network handling is the caller's
responsibility.
...
To publish a `files_daf` over HTTP, the directory needs a
`metadata.zip` bundle at its root. From dafr 0.2.0 onward this is
maintained automatically by every write; for pre-0.2.0 stores call
...
verified for axes, dense and sparse vectors, dense and sparse
