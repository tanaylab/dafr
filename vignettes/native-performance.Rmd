...
the axis. It accepts named integer permutations (`new_entries[i] =
old_entries[perm[i]]`):
...
`reorder_axes()` only operates on leaf storage formats (`memory_daf`,
`files_daf`, `zarr_daf`, `http_daf` — though HTTP is read-only).
Wrappers (`chain_reader`, `chain_writer`, `view`, `contractor`) are
rejected up front; reorder them via the underlying writer instead.
