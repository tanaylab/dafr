...
QFILE <- "/home/aviezerl/src/dafr-native/dev/adversarial-parity/queries_all.txt"
OUT   <- "/home/aviezerl/src/dafr-native/dev/adversarial-parity/r_out.jsonl"
...
serialize_result <- function(x) {
    ...
    if (is.matrix(x) || inherits(x, "Matrix")) {
        ...
        return(list(
            ...
            dim = dim(m),
            rownames = if (is.null(rn)) NA else rn,
            colnames = if (is.null(cn)) NA else cn,
            values = vals
        ))
    }
